...
import com.prj.common.util.JsonUtils;

/**
 * 
 * @author yiliang.gyl
 * @version $Id: TokenHandler.java, v 0.1 Jun 21, 2015 11:01:12 AM yiliang.gyl Exp $
 */
@Component
public class TokenHandler {
    ...
    public AccountUserDetails parseAccountFromToken(String token) {
        ...
        if (parts.length == 2 && parts[0].length() > 0 && parts[1].length() > 0) {
            ...
            if (validHash) {
                final AccountUserDetails details = (AccountUserDetails) JsonUtils.deserialize(
                    new String(userBytes), new TypeReference<AccountUserDetails>() {
                    });
                return details;
            }
        }
